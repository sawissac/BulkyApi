...
function parseSseBody(text: string): ParsedSseEvent[] {
  ...
  for (const line of lines) {
    if (line === '') {
      ...
      type = 'message'; data = ''; id = undefined;
...
function SseBodyEvents({ T, events, raw, onToggleRaw, response }: {
  T: Theme;
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
        ...
        <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 8, fontWeight: 700, letterSpacing: '0.1em', color: SSE_CLR }}>
          SSE
        </span>
        <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 8, color: T.textDim }}>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 3 }}>
          {(['SSE', 'RAW'] as const).map((label) => {
            ...
            return (
              <button
                key={label}
                onClick={() => { if (wantsRaw !== raw) onToggleRaw(); }}
                style={{
                  padding: '2px 8px', borderRadius: 9999, cursor: active ? 'default' : 'pointer',
                  border: `1px solid ${active ? SSE_CLR : T.border}`,
                  background: active ? `${SSE_CLR}15` : 'transparent',
                  color: active ? SSE_CLR : T.textDim,
                  fontFamily: "'Space Grotesk', sans-serif", fontSize: 8, fontWeight: 700, letterSpacing: '0.1em',
                }}
              >
                {label}
              </button>
      ...
      {raw ? (
        <pre style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: T.text, whiteSpace: 'pre-wrap', wordBreak: 'break-word', background: T.bgHover, border: `1px solid ${T.border}`, borderRadius: 6, padding: 10, margin: 0 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          {events.map((ev, i) => {
            ...
            try { parsed = JSON.parse(ev.data); } catch { /* string */ }
            const isMsg = ev.type === 'message';
            return (
              <div key={i} style={{ borderRadius: 5, border: `1px solid ${SSE_CLR}20`, background: `${SSE_CLR}08`, padding: '5px 8px', display: 'flex', flexDirection: 'column', gap: 3 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 7, fontWeight: 700, color: T.textDim }}>#{i + 1}</span>
                  {!isMsg && (
                    <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 7, fontWeight: 700, letterSpacing: '0.08em', color: SSE_CLR, background: `${SSE_CLR}18`, border: `1px solid ${SSE_CLR}30`, padding: '1px 5px', borderRadius: 3 }}>
                      {ev.type}
                    </span>
                  )}
                  {ev.id && (
                    <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 7, color: T.textDim }}>id: {ev.id}</span>
                  )}
                </div>
                {typeof parsed === 'string' ? (
                  <pre style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: T.text, margin: 0, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
                    {parsed}
                  </pre>
                ) : (
...
function SseEvents({ T, call }: Props) {
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
        <Radio size={10} color={SSE_CLR} style={isStreaming && events.length === 0 ? { animation: 'pulse 1s ease-in-out infinite' } : undefined} />
        <span style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 8, fontWeight: 700, letterSpacing: '0.1em', color: SSE_CLR }}>
          SSE STREAM
        </span>
        <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 8, color: T.textDim }}>
      ...
      {events.length === 0 ? (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: T.textDim, padding: '8px 0' }}>
          {call.status === 'pending' && <Loader2 size={11} color={SSE_CLR} style={{ animation: 'spin 0.7s linear infinite' }} />}
          <span style={{ fontFamily: "'Poppins', sans-serif", fontSize: 11, fontStyle: 'italic' }}>
            ...
            return (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <span style={{ marginLeft: 'auto', fontFamily: "'JetBrains Mono', monospace", fontSize: 7, color: T.textDim }}>
...
export default function RespTab({ T, call }: Props) {
  ...
  if (isSseBody(call.response)) {
    ...
    return <SseBodyEvents T={T} events={events} raw={raw} onToggleRaw={() => setRaw(!raw)} response={call.response} />;
  }
